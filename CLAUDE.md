...
- **Braces**: always use curly braces for `if`/`else`/`for`/`while` bodies, even single-line. Never place the body on the same line as the opening brace — always line-break after `{`. Enforced by `.clang-format` (`AllowShortIfStatementsOnASingleLine: Never`). Run `clang-format -i` on any edited file before committing.
