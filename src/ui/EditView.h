...
class EditView {
 public:
  ...
  EditView(catalog::PhotoRepository& repo, catalog::ThumbnailCache& thumbCache,
 ...
 private:
  ...
  int aspectMode_ = 1;  // 0=Free,1=Original,2=1:1,3=2:3,4=3:2
  ...
  std::vector<uint8_t> applyEditsToPixels(const std::vector<uint8_t>& src, int w, int h,
  ...
  void regenThumbnail(int64_t photoId, catalog::EditSettings s, std::vector<uint8_t> srcRgb,
                      int srcW, int srcH);
  ...
  bool renderSliderRow(const char* label, float* v, float vmin, float vmax, float step);
