...
class TextureManager {
 public:
  ...
  static bool decodeJpeg(const std::vector<uint8_t>& jpegBytes, std::vector<uint8_t>& outRgba,
                         int& outW, int& outH);
 ...
 private:
  ...
  std::unordered_map<int64_t, std::pair<std::list<int64_t>::iterator, TextureEntry>> lruMap_;
  mutable std::mutex mutex_;

  void evictOldest();
  MTLTexturePtr makePlaceholder();
  MTLTexturePtr decodeAndCreate(const std::vector<uint8_t>& jpegBytes, int& outW, int& outH);
};
