...
void FullscreenView::setPhotoList(std::vector<int64_t> ids, int64_t currentId) {
    ...
    currentIdx_ = (it != photoIds_.end())
                  ? static_cast<int>(it - photoIds_.begin()) : 0;
}
...
void FullscreenView::tickToast(ImDrawList* dl, ImVec2 scrSz, float dt) {
    toastTimeLeft_ -= dt;
    if (toastTimeLeft_ <= 0.f) { toastVisible_ = false; return; }
...
void FullscreenView::render() {
    ...
    if (toastVisible_) tickToast(dl, scrSz, io.DeltaTime);
}
