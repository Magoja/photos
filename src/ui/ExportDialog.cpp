...
void ExportDialog::render() {
    ...
    if (!ImGui::Begin("Export Photos##dlg", &open_)) { ImGui::End(); return; }
    ...
    if (!exporting_ && !finished_) {
        ...
        if (!presets_.empty())
            std::snprintf(buf, sizeof(buf), "%s",
                         presets_[selectedPreset_].targetPath.c_str());
        ...
        if (ImGui::Button("Cancel##exp")) exporter_->cancel();

    } else {
        ImGui::TextColored({0.2f,1.f,0.2f,1.f},
            "Export complete! %d exported, %d errors",
