...
MTLTexturePtr TextureManager::makePlaceholder() {
    ...
    for (int i = 3; i < W * H * 4; i += 4) pixels[i] = 255;
...
MTLTexturePtr TextureManager::decodeAndCreate(const std::vector<uint8_t>& jpegBytes,
                                               int& outW, int& outH)
{
...
MTLTexturePtr TextureManager::get(int64_t photoId) {
    ...
    if (it == lruMap_.end()) return placeholder_;
