...
void FolderTreePanel::render() {
    ...
    if (selectedFolder_ == 0) allFlags |= ImGuiTreeNodeFlags_Selected;
    ...
    if (volumes_.empty()) {
        ...
        for (auto& vol : volumes_) {
            ...
            std::snprintf(volLabel, sizeof(volLabel), "[Disk] %s##v%lld",
                          vol.label.empty() ? "Untitled" : vol.label.c_str(),
                          (long long)vol.id);
            bool volOpen = ImGui::TreeNodeEx(volLabel,
