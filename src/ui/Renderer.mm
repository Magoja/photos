...
bool Renderer::init(SDL_Window* window) {
  ...
  impl_->rpd.colorAttachments[0].clearColor = MTLClearColorMake(0.12, 0.12, 0.12, 1.0);
...
void Renderer::beginFrame() {
  ...
  impl_->layer.drawableSize = CGSizeMake((CGFloat)io.DisplaySize.x * io.DisplayFramebufferScale.x,
...
void Renderer::endFrame() {
  ...
  ImGui_ImplMetal_RenderDrawData(ImGui::GetDrawData(), impl_->currentCmdBuf, enc);
  [enc endEncoding];
  [impl_->currentCmdBuf presentDrawable:impl_->currentDrawable];
  [impl_->currentCmdBuf commit];

  auto now = std::chrono::steady_clock::now();
  frameTimeMs_ = std::chrono::duration<float, std::milli>(now - impl_->lastFrameTime).count();
