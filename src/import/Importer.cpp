...
static std::optional<int64_t> dbDuplicateCheck(Database& db,
...
static PhotoRecord buildPhotoRecord(const DecodeResult& dec,
                                     int64_t folderId,
    ...
    for (auto& sf : files) {
        ...
        try {
            ...
            if (opts_.copyFiles) {
                ...
                if (!result) { ++stats_.errors; continue; }
                destFile = *result;
            } else {
                destFile = sf.path;
            }
            ...
            auto p = buildPhotoRecord(dec, fid,
                                      fs::path(destFile).filename().string(),
                                      hash, sf.size);
            int64_t pid;
            {
                std::lock_guard lk(db_.mutex());
