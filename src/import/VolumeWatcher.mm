...
static void onDiskAppeared(DADiskRef disk, void* ctx)
{
    ...
    if (!urlRef) { CFRelease(desc); return; }
    ...
    if (info.uuid.empty()) { CFRelease(desc); return; }
    ...
    // Mount path
    char urlBuf[1024] = {};
    CFURLGetFileSystemRepresentation((CFURLRef)urlRef, true,
                                     (UInt8*)urlBuf, sizeof(urlBuf));
...
void VolumeWatcher::start() {
    ...
    DASessionScheduleWithRunLoop(impl_->session,
                                 CFRunLoopGetCurrent(),
    ...
    DARegisterDiskAppearedCallback(impl_->session, match,
                                   onDiskAppeared, this);
    DARegisterDiskDisappearedCallback(impl_->session, match,
                                      onDiskDisappeared, this);
    CFRelease(match);
...
void VolumeWatcher::stop() {
    ...
    DASessionUnscheduleFromRunLoop(impl_->session,
                                   CFRunLoopGetCurrent(),
                                   kCFRunLoopDefaultMode);
...
void VolumeWatcher::onMounted(const VolumeInfo& info) {
    spdlog::info("Volume mounted uuid={} path={} label={}",
                 info.uuid, info.mountPath, info.label);
    if (mountedCb_) mountedCb_(info);
}

void VolumeWatcher::onUnmounted(const std::string& uuid) {
    spdlog::info("Volume unmounted uuid={}", uuid);
    if (unmountedCb_) unmountedCb_(uuid);
}
