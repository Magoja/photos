...
static RgbImage decodeWithLibRaw(const std::string& path, int maxEdge) {
  ...
  if (raw->open_file(path.c_str()) != LIBRAW_SUCCESS || raw->unpack() != LIBRAW_SUCCESS ||
  ...
  if (maxEdge > 0 && std::max(img->width, img->height) > maxEdge) {
    ...
    result.pixels =
      downsampleRgb(img->data, img->width, img->height, scale, result.width, result.height);
  } else {
...
static RgbImage decodeWithTurboJpeg(const std::string& path, int maxEdge) {
  ...
  if (tjDecompressHeader3(tj, bytes.data(), static_cast<unsigned long>(sz), &srcW, &srcH, &subsamp,
                          &cs) < 0) {
    return {};
  }

  std::vector<uint8_t> rgb(static_cast<size_t>(srcW * srcH * 3));
  if (tjDecompress2(tj, bytes.data(), static_cast<unsigned long>(sz), rgb.data(), srcW, 0, srcH,
  ...
  if (maxEdge > 0 && std::max(srcW, srcH) > maxEdge) {
    ...
    result.pixels = downsampleRgb(rgb.data(), srcW, srcH, scale, result.width, result.height);
  } else {
