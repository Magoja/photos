...
CommandRegistry buildRegistry(catalog::PhotoRepository& repo, ui::TextureManager& texMgr,
                              ui::GridView& grid, export_ns::ExportSession& session) {
  ...
  registry.registerHandler("image.adjust", std::make_unique<ImageAdjustHandler>(repo, evictBoth));
  registry.registerHandler("image.revert", std::make_unique<ImageRevertHandler>(repo, evictBoth));
  registry.registerHandler("image.crop", std::make_unique<ImageCropHandler>(repo));
  registry.registerHandler("image.save", std::make_unique<ImageSaveHandler>(repo, nullptr));
