...
std::vector<int64_t> PhotoRepository::queryByFolder(int64_t folderId, bool pickedOnly) {
  Stmt s =
    pickedOnly
      ? db_.prepare(
          "SELECT id FROM photos WHERE folder_id=? AND picked=1 ORDER BY "
          "COALESCE(capture_time,import_time)")
      : db_.prepare(
          "SELECT id FROM photos WHERE folder_id=? ORDER BY COALESCE(capture_time,import_time)");
  s.bind(1, folderId);
  return collectIds(s);
}
...
void PhotoRepository::updateThumb(int64_t id, const std::string& path, int w, int h,
                                  int64_t mtime) {
  auto s = db_.prepare(
