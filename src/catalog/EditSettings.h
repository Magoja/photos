...
struct EditSettings {
  ...
  bool isIdentity() const {
    return exposure == 0.f && temperature == 0.f && contrast == 0.f && saturation == 0.f &&
           crop.x == 0.f && crop.y == 0.f && crop.w == 1.f && crop.h == 1.f && crop.angleDeg == 0.f;
  }
  ...
  std::string toJson() const {
    ...
    j["crop"] = {
      {"x", crop.x}, {"y", crop.y}, {"w", crop.w}, {"h", crop.h}, {"angle", crop.angleDeg}};
    return j.dump();
  }
