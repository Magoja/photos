...
TEST_CASE("Insert photo and find by hash", "[db]") {
    ...
    VolumeRecord v; v.uuid = "U1"; v.label = "L"; v.mountPath = "/V";
    int64_t vid = repo.upsertVolume(v);
    FolderRecord folder; folder.volumeId = vid; folder.path = "/V/DCIM"; folder.name = "DCIM";
    int64_t fid = repo.upsertFolder(folder);
...
TEST_CASE("Duplicate (folder_id, filename) is rejected", "[db]") {
    ...
    VolumeRecord v; v.uuid = "U2"; v.label = "L2"; v.mountPath = "/V2";
    int64_t vid = repo.upsertVolume(v);
    FolderRecord folder; folder.volumeId = vid; folder.path = "/V2/DCIM"; folder.name = "DCIM";
    int64_t fid = repo.upsertFolder(folder);
...
TEST_CASE("queryByFolder returns correct IDs", "[db]") {
    ...
    VolumeRecord v; v.uuid = "U3"; v.label = "L3"; v.mountPath = "/V3";
    int64_t vid = repo.upsertVolume(v);
    FolderRecord folder; folder.volumeId = vid; folder.path = "/V3/A"; folder.name = "A";
    int64_t fid = repo.upsertFolder(folder);
...
TEST_CASE("updatePicked and filter", "[db]") {
    ...
    VolumeRecord v; v.uuid = "U4"; v.label = "L4"; v.mountPath = "/V4";
    int64_t vid = repo.upsertVolume(v);
    FolderRecord folder; folder.volumeId = vid; folder.path = "/V4/B"; folder.name = "B";
    int64_t fid = repo.upsertFolder(folder);

    PhotoRecord p; p.folderId = fid; p.filename = "a.jpg"; p.fileSize = 1;
    int64_t pid = repo.insertPhoto(p);
