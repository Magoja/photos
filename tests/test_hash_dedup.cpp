...
TEST_CASE("Identical file produces same fast fingerprint", "[hash]") {
    auto p = writeTempFile("same.bin",
        std::string(128 * 1024, 'A')); // > 2 chunks
...
TEST_CASE("isDuplicate returns photo_id for known hash", "[hash]") {
    ...
    VolumeRecord v; v.uuid = "DU1"; v.label = "L"; v.mountPath = "/DU1";
    int64_t vid = repo.upsertVolume(v);
    FolderRecord folder; folder.volumeId = vid; folder.path = "/DU1/A"; folder.name = "A";
